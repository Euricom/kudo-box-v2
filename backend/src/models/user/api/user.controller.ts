...
export class UserController {
    ...
    async getLoggedInKudos(@Request() req: RequestWithUser): Promise<MyKudosDto> {
        ...
        const receivedKudosDtos = await Promise.all(kudos.filter(k => {
            return (k.event && k.event.host && k.event.host.id === req.user.toUpperCase()) || (k.receiver && k.receiver.id === req.user.toUpperCase());
        })
